...
public class BenchThread implements Runnable {
	...
	while (!stop) {
	    ...
	    int operationNumber;
	    OperationExecutor currentExecutor = OperationScheduler.getScheduler().getNextOperation();
...
//	    System.out.println(OperationId.values()[currentExecutor.getOperationNumber()].toString());
	    int result = 0;
...
//		currentExecutor.setRunner(this);
	    ...
	    if (Parameters.sequentialReplayEnabled) {
//	    	System.out.println(currentExecutor.getClass() + " " + currentExecutor.getOperationNumber()+ " " +operationNumber);
