...
public class Benchmark {
	...
	while (argNumber < args.length) {
		...
		else if (currentArg.equals("--no-shorttraversals"))
		    Parameters.shortTraversalsEnabled = false;
...
//		   operations = new OperationExecutor[Parameters.numOps];
...
//		    Transaction.init(OperationId.values().length, args[argNumber++]);
			...
			else if (currentArg.equals("-futures"))
			    Parameters.numSubThreads = Integer.parseInt(optionValue);
...
//	if (Parameters.shortTraversalsEnabled)
//		shortTraversalsRatio = Parameters.ShortTraversalsRatio / 100.0;
//	else
//		shortTraversalsRatio = 0;
	...
	operationCDF = new double[operations.length];//vecor que contem as probabilidades para cada familia de operações
...
//		OperationExecutor[] threadOperations = new OperationExecutor[Parameters.numOps/Parameters.numThreads];
//		for(int i =0; i < Parameters.numOps/Parameters.numThreads;i++) {
//			threadOperations[i] = operations[firstOperation];
//			firstOperation++;
//		}
	...
	OperationScheduler.getScheduler().setupStructures(operationCDF,operations);
	OperationScheduler.getScheduler().replenishOperations(Parameters.numOps);
...
//    }
	...
	int i = 0;
	System.out.println(Parameters.numOps + " operations");
	System.out.println(Parameters.numSubThreads + " subthreads");
	System.out.println(Parameters.numThreads + " threads");
	System.out.println(Parameters.parNest + " nested?");
	System.out.println(Parameters.workloadType + " type");
	System.out.println(Parameters.structureModificationEnabled + " structural?");
	...
	OperationScheduler.getScheduler().setupStructures(operationCDF,operations);
	OperationScheduler.getScheduler().replenishOperations(Parameters.numOps);
	SequentialReplayThread seqThread = new SequentialReplayThread(setupClone, operationCDF, replayLog, operations);
	seqThread.run();
	setFactoryInstances(synchMethodInitializer);
	...
	try {
        ...
        out.write(Math.round(elapsedTime) + "," +JVSTMStats.topLevelAborts.get() + "," + Transaction.nestedValidationAbort.get() + "," + Transaction.nestedabort.get() + ","+Transaction.futuresCreated.get()+ "," + Transaction.successfullValidations.get()+ "," );
        out.close();
    }catch (IOException e) {
		e.printStackTrace();
	}
