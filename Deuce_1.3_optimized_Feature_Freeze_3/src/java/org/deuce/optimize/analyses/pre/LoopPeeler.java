 ...
 */
public class LoopPeeler extends BodyTransformer {
  /**
...
/**
  ...
  protected void internalTransform(Body body, String phaseName, Map options) {
    ...
    this.body = body;
    this.maxSize = options.containsKey("maxSize") ? PhaseOptions.getInt(options, "maxSize") : 5;
  ...
  private int getSize(Block block) {
    ...
    for (Unit unit = block.getHead(); unit != getBlockTail(block);
  ...
  private Unit copyBlock(Block block) {
    ...
    Unit tail = getBlockTail(block);
    Unit immediateSucc = unitChain.getSuccOf(tail);
    Unit newGoto = manipulations.insertGotoAfter(tail, immediateSucc);
  ...
  private void unrollConditions(Block block) {
    ...
    Iterator<Block> succsIt = block.getSuccs().iterator();
        ...
        if (succ != block && visitingSuccs.contains(succ)) {
          ...
          if (succ.getPreds().size() >= 2 && succ.getSuccs().size() == 2) {
            ...
            if (getSize(condition) <= maxSize) {
              ...
              /* now just redirect the tail of the loop-body to the new condition block. */
              Unit loopTail = getBlockTail(loopTailBlock);
              manipulations.redirectExitToNewTarget(condition.getHead(), copiedHead, loopTail);

          	  if (condition.getSuccs().contains(loopTailBlock)) {
          		G.v().out.println("Peeling loop at " + body.getMethod().toString());
//          		if (false ||
//          				body.getMethod().toString().contains("jstamp.KMeans.KMeans")
//          				body.getMethod().toString().contains("jstamp.KMeans.Cluster")
          				//body.getMethod().toString().contains("jstamp.KMeans.Normal")
//          				body.getMethod().toString().contains("extractMoments2")
//          				|| body.getMethod().toString().contains("zscoreTransform")
//          				)
//          		{
//          			System.out.println("skipping PEELING method:" + body.getMethod().toString());
//          		}else
          		{
	        		Unit copiedBodyHead = copyBlock(loopTailBlock);
	        		// need to make condition point to body.
	        		Unit conditionTail = getBlockTail(condition);
	        		// we know that condition has exactly 2 predecessors.
	        		// so conditionTail is probably an if statement.
	        		manipulations.redirectExitToNewTarget(loopTailBlock.getHead(), 
	        				copiedBodyHead, conditionTail);
          		}
        	  }
            }
          }
	...
	private Unit getBlockTail(Block loopTailBlock) {
		return loopTailBlock.getTail();
	}
