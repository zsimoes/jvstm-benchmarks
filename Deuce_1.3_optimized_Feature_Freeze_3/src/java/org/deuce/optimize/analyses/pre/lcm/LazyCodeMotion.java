...
import org.deuce.optimize.analyses.pre.LoopPeeler;
import org.deuce.optimize.analyses.pre.sp.ScalarPromotion;
...
public class LazyCodeMotion extends BodyTransformer {
  ...
  protected void internalTransform(Body b, String phaseName, Map opts) {
    ...
    HashMap<EquivalentValue, Local> expToHelper = new LinkedHashMap<EquivalentValue, Local>();

    if(Options.v().verbose()) G.v().out.println("[" + b.getMethod().getName() +
                                          "] Performing Lazy Code Motion...");

   if (!NotAlwaysAtomicDatabase.getInstance().isMethodAlwaysAtomic(b.getMethod()))
	   return;
    
    if (options.unroll()) {
		LoopPeeler loopPeeler = new LoopPeeler();
		loopPeeler.transform(b, phaseName + ".lcu");
		ScalarPromotion sp = new ScalarPromotion();
		sp.transform(b, phaseName + "sp");
	}
    ...
    int maBefore = countMemoryAccesses(b);
    doLCM(b, options, expToHelper, unitChain);
    int maAfter = countMemoryAccesses(b);
    int diff = maBefore - maAfter;

    if (diff > 0) {
    	G.v().out.println("LCM: " + b.getMethod().toString() + ": " + diff + " memory accesses were removed " +
    		"(before=" + maBefore +", after=" + maAfter + ")");
    	Integer num = (Integer)opts.get("memory-accesses-removed");
    	num += diff;
    	opts.put("memory-accesses-removed", num);
    }
  }
  
  
private void doLCM(Body b, LCMOptions options,
		HashMap<EquivalentValue, Local> expToHelper, Chain<Unit> unitChain) {
	UnitGraph graph = new BriefUnitGraph(b);

    /* map each unit to its RHS. only take binary expressions */
    Map<Unit, Value> unitToEquivRhs = new UnitMap(b, graph.size() + 1, 0.7f) {
	protected Value mapTo(Unit unit) {
    ...
    Map<Unit, Value> unitToNoExceptionEquivRhs = new UnitMap(b, graph.size() + 1, 0.7f) {
	protected Value mapTo(Unit unit) {
    ...
    FlowUniverse<Value> universe = new CollectionFlowUniverse<Value>(unitToEquivRhs.values());
    ...
    */
    ...
    { /* replace old computations by the helper-vars */
      ...
      while (unitIt.hasNext()) {
	    ...
	    try {
	    	G.v().out.println("LCM: " + b.getMethod().toString() + ": replacing assignment " + currentUnit.toString() + " with a local " + helper.toString());
	...
	private int countMemoryAccesses(Body b) {
		...
		for (Unit unit : units) {			
			if (unit instanceof AssignStmt)
			{
				...
				if (rightOp instanceof ConcreteRef)
				{
					// found x = o.f or x = a[i]
					counter++;
				}
			}
		}
