...
public class EarliestnessComputation {
  ...
  public EarliestnessComputation(UnitGraph unitGraph, UpSafetyAnalysis upSafe,
    ...
    unitToEarliest = new LinkedHashMap<Unit, FlowSet>(unitGraph.size() + 1, 0.7f);
	...
	while (unitIt.hasNext()) {
	  ...
	  if(!unitToEarliest.containsKey(currentUnit))
	  {
	  unitToEarliest.put(currentUnit, earliest);
	  }
	  else
	  {
		  earliest = unitToEarliest.get(currentUnit);
	  }
