 ...
 * More precise: The delayability-analysis tells us already until which point we
...
public class LatestComputation {
  ...
  public LatestComputation(UnitGraph unitGraph, DelayabilityAnalysis delayed,
    ...
    unitToLatest = new LinkedHashMap<Unit, FlowSet>(unitGraph.size() + 1, 0.7f);

    iterateUnits(unitGraph, delayed, equivRhsMap, set);
  }

private void iterateUnits(UnitGraph unitGraph, DelayabilityAnalysis delayed,
		Map equivRhsMap, BoundedFlowSet set) {
