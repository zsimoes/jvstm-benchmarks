...
public class StaticInliner extends SceneTransformer {
	...
	protected void internalTransform(String phaseName, Map options) {
		...
		options.put("rerun-jb", "true");
		options.put("enabled", "true");
		options.put("allowed-modifier-changes", "unsafe");
		options.put("max-container-size", "5000");
		options.put("max-inlinee-size", "20");
		options.put("insert-redundant-casts", "true");
		options.put("insert-null-checks", "false");
		options.put("expansion-factor", "3");
		...
		float expansionFactor = PhaseOptions.getFloat(options,
				"expansion-factor");
		int maxContainerSize = PhaseOptions.getInt(options,
				"max-container-size");
		...
		// Visit each potential site in reverse pseudo topological order.
		{
			...
			while (it.hasPrevious()) {
				...
				if (!explicitInvokesFilter.wrap(cg.edgesOutOf(container))
						.hasNext())
					continue;
				...
				while (unitIt.hasNext()) {
					...
					if (!InlinerSafetyManager.ensureInlinability(target, s,
							container, modifierOptions))
						continue;
		...
		// expansion rates.
		{
			...
			while (sitesIt.hasNext()) {
				...
				int inlineeSize = ((JimpleBody) (inlinee.retrieveActiveBody()))
						.getUnits().size();

				Stmt invokeStmt = (Stmt) l.get(1);

				SootMethod container = (SootMethod) l.get(2);
				int containerSize = ((JimpleBody) (container
				...
				if (inlineeSize + containerSize > expansionFactor
						* methodToOriginalSize.get(container).intValue())
					continue;

				if (InlinerSafetyManager.ensureInlinability(inlinee,
						invokeStmt, container, modifierOptions)) {
					...
					if (rerunJb) {
						PackManager.v().getPack("jb")
								.apply(container.getActiveBody());
					}
				}
			}
		}
		methodToOriginalSize.clear();
	}
	...
	private void computeAverageMethodSizeAndSaveOriginalSizes() {
		...
		while (classesIt.hasNext()) {
			...
			while (methodsIt.hasNext()) {
				...
				if (m.isConcrete()) {
					int size = ((JimpleBody) m.retrieveActiveBody()).getUnits()
							.size();
					sum += size;
