...
public class InlinerSafetyManager {
	...
	public static boolean checkSpecialInlineRestrictions(SootMethod container,
			SootMethod target, String options) {
			...
			if (st.containsInvokeExpr()) {
				...
				if (ie1 instanceof SpecialInvokeExpr) {
					if ((InlinerSafetyManager.specialInvokePerformsLookupIn(
							ie1, container.getDeclaringClass()) || InlinerSafetyManager
							.specialInvokePerformsLookupIn(ie1,
					...
					if (specialTarget.isPrivate()) {
						if (specialTarget.getDeclaringClass() != container
								.getDeclaringClass()) {
	...
	public static boolean checkAccessRestrictions(SootMethod container,
			SootMethod target, String modifierOptions) {
...
//   method or field access restrictions
		{
			...
			while (unitsIt.hasNext()) {
				...
				if (st.containsInvokeExpr()) {
					...
					if (!AccessManager.ensureAccess(container, ie1.getMethod(),
							modifierOptions))
						return false;
				}
				...
				if (st instanceof AssignStmt) {
					...
					if (lhs instanceof FieldRef
							&& !AccessManager.ensureAccess(container,
									((FieldRef) lhs).getField(),
									modifierOptions))
						return false;
	...
	*/
	...
	public static boolean ensureInlinability(SootMethod target, Stmt toInline,
			SootMethod container, String modifierOptions) {
		if (!InlinerSafetyManager.canSafelyInlineInto(target, toInline,
				container)) {
	...
	private static boolean canSafelyInlineInto(SootMethod inlinee,
			Stmt toInline, SootMethod container)

	{
		...
		Value base = (ie instanceof InstanceInvokeExpr) ? ((InstanceInvokeExpr) ie)
				.getBase() : null;

		if (base != null
				&& base.getType() instanceof RefType
				&& invokeThrowsAccessErrorIn(
						((RefType) base.getType()).getSootClass(), inlinee,
						container))
			return false;
		...
		if (ie instanceof SpecialInvokeExpr
				&& (specialInvokePerformsLookupIn(ie,
						inlinee.getDeclaringClass()) || specialInvokePerformsLookupIn(
						ie, container.getDeclaringClass())))
			return false;
	...
	private static boolean invokeThrowsAccessErrorIn(SootClass base,
			SootMethod inlinee, SootMethod container) {
		...
		if (inlinee.isProtected()) {
			...
			if (h.isClassSuperclassOfIncluding(inlineeClass, containerClass)
					|| ((base != null) && h.isClassSuperclassOfIncluding(base,
							containerClass)))
				saved = true;
	...
	// is being called.
	static boolean specialInvokePerformsLookupIn(InvokeExpr ie,
