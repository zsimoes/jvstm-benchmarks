...
public class AccessManager {
	...
	public static boolean isAccessLegal(SootMethod container, SootClass target) {
		return target.isPublic()
				|| container.getDeclaringClass().getPackageName()
	...
	public static boolean isAccessLegal(SootMethod container, Stmt stmt) {
		if (stmt.containsInvokeExpr()) {
			return AccessManager.isAccessLegal(container, stmt.getInvokeExpr()
					.getMethod());
...
//    			 set
	...
	private static void createGetAccessor(SootMethod container, AssignStmt as,
			FieldRef ref) {
		...
		if (target.declaresMethodByName(name)) {
			...
			if (ref instanceof InstanceFieldRef) {
				...
				accStmts.addFirst(Jimple.v().newIdentityStmt(thisLocal,
			...
			if (ref instanceof InstanceFieldRef) {
				v = Jimple.v()
						.newInstanceFieldRef(thisLocal, ref.getFieldRef());
			} else {
			...
			accessor = new SootMethod(name, parameterTypes, returnType,
					Modifier.PUBLIC | Modifier.STATIC, thrownExceptions);
		...
		InvokeExpr newExpr = Jimple.v().newStaticInvokeExpr(accessor.makeRef(),
				args);

		as.setRightOp(newExpr);
	}

	private static void createSetAccessor(SootMethod container, AssignStmt as,
			FieldRef ref) {
		java.util.List parameterTypes = new LinkedList();
		java.util.List<SootClass> thrownExceptions = new LinkedList<SootClass>();
		...
		if (target.declaresMethodByName(name)) {
			...
			int paramID = 0;
			if (ref instanceof InstanceFieldRef) {
				accStmts.add(Jimple.v().newIdentityStmt(thisLocal,
			...
			Local l = lg.generateLocal(ref.getField().getType());
			accStmts.add(Jimple.v().newIdentityStmt(
					l,
					Jimple.v().newParameterRef(ref.getField().getType(),
							paramID)));
			paramID++;
			if (ref instanceof InstanceFieldRef) {
	...
	private static void createInvokeAccessor(SootMethod container, Stmt stmt) {
		...
		if (target.declaresMethodByName(name)) {
			...
			int paramID = 0;
			for (java.util.Iterator it = parameterTypes.iterator(); it
					.hasNext();) {
				Type type = (Type) it.next();
				Local l = lg.generateLocal(type);
				//System.out.println("local type: " + type);
				accStmts.add(Jimple.v().newIdentityStmt(l,
			...
			if (expr instanceof StaticInvokeExpr) {
				accExpr = Jimple.v().newStaticInvokeExpr(method.makeRef(),
						arguments);
			} else if (expr instanceof VirtualInvokeExpr) {
				Local thisLocal = (Local) arguments.get(0);
				arguments.remove(0);
				accExpr = Jimple.v().newVirtualInvokeExpr(thisLocal,
						method.makeRef(), arguments);
			} else if (expr instanceof SpecialInvokeExpr) {
				Local thisLocal = (Local) arguments.get(0);
				arguments.remove(0);
				accExpr = Jimple.v().newSpecialInvokeExpr(thisLocal,
						method.makeRef(), arguments);
			} else
				throw new RuntimeException("");
	...
	public static boolean ensureAccess(SootMethod container,
			ClassMember target, String options) {
	...
	public static boolean ensureAccess(SootMethod container, SootClass target,
			String options) {
		boolean accessors = options.equals("accessors");
		boolean allowChanges = !(options.equals("none"));
		boolean safeChangesOnly = !(options.equals("unsafe"));
