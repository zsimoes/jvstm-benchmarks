...
package org.deuce.optimize.analyses.inline;
...
public class SiteInliner {
	...
	public static List inlineSite(SootMethod inlinee, Stmt toInline,
		...
		boolean enableNullPointerCheckInsertion = PhaseOptions.getBoolean(
				options, "insert-null-checks");
		boolean enableRedundantCastInsertion = PhaseOptions.getBoolean(options,
		...
		if (!(inlinee.getDeclaringClass().isApplicationClass() || inlinee
				.getDeclaringClass().isLibraryClass()))
			return null;
		...
		// Insert casts to please the verifier.
		{
			...
			if (enableRedundantCastInsertion
					&& ie instanceof InstanceInvokeExpr && targetUsesThis) {
				...
				localType = ((RefType) ((InstanceInvokeExpr) ie).getBase()
						.getType()).getSootClass();
				parameterType = inlinee.getDeclaringClass();

				if (localType.isInterface()
						|| hierarchy.isClassSuperclassOf(localType,
								parameterType)) {
					Local castee = Jimple.v().newLocal("__castee",
		...
		// (If enabled), add a null pointer check.
		{
			if (enableNullPointerCheckInsertion
				...
				boolean caught = TrapManager.isExceptionCaughtAt(Scene.v()
						.getSootClass("java.lang.NullPointerException"),
						toInline, containerB);
				...
				if (caught) {
					...
					Stmt throwPoint = ThrowManager
							.getNullPointerExceptionThrower(containerB);
					containerB
							.getUnits()
							.insertBefore(
									Jimple.v()
		...
		// Add synchronizing stuff.
		{
			if (inlinee.isSynchronized()) {
				...
				if (ie instanceof InstanceInvokeExpr)
					SynchronizerManager.v().synchronizeStmtOn(toInline,
							containerB,
					...
					if (!container.getDeclaringClass().isInterface()) {
						// Whew!
						Local l = SynchronizerManager.v()
								.addStmtsToFetchClassBefore(containerB,
										toInline);
						SynchronizerManager.v().synchronizeStmtOn(toInline,
								containerB, l);
					}
		...
		// Copy & backpatch the traps; preserve their same order.
		{
			...
			while (trapsIt.hasNext()) {
				...
				Stmt newBegin = oldUnitsToNew.get(t.getBeginUnit()), newEnd = oldUnitsToNew
						.get(t.getEndUnit()), newHandler = oldUnitsToNew.get(t
						.getHandlerUnit());

				if (newBegin == null || newEnd == null || newHandler == null)
					throw new RuntimeException("couldn't map trap!");

				Trap trap = Jimple.v().newTrap(t.getException(), newBegin,
						newEnd, newHandler);
				if (prevTrap == null)
		...
		// Handle identity stmt's and returns.
		{
			...
			while (it.hasNext()) {
				...
				if (s instanceof IdentityStmt) {
					IdentityRef rhs = (IdentityRef) ((IdentityStmt) s)
							.getRightOp();
					if (rhs instanceof CaughtExceptionRef)
						...
						if (!(ie instanceof InstanceInvokeExpr))
							throw new RuntimeException(
									"thisref with no receiver!");

						containerUnits.swapWith(
								s,
								Jimple.v().newAssignStmt(
										((IdentityStmt) s).getLeftOp(),
										thisToAdd));
					...
					if (toInline instanceof InvokeStmt) {
						// munch, munch.
						containerUnits.swapWith(s,
								Jimple.v().newGotoStmt(exitPoint));
						continue;
					}
					...
					containerUnits.swapWith(s, Jimple.v()
							.newGotoStmt(exitPoint));
		...
		for (Iterator i = containerUnits.iterator(
				containerUnits.getSuccOf(toInline),
				containerUnits.getPredOf(exitPoint)); i.hasNext();) {
			newStmts.add(i.next());
		}
