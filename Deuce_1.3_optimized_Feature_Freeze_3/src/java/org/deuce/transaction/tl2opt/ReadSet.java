...
public class ReadSet{
	...
	public ReadFieldAccess getNextNoIncrement() {
		currentReadFieldAccess = readSet[0];
		return currentReadFieldAccess;
	}
	
    public void checkClock(int clock) {
        for (int i = 0; i < nextAvaliable; i++) {
        	ReadFieldAccess readFieldAccess = readSet[i];
        	
        	boolean threadLocal = (readFieldAccess.getAdvice() & Advisor.THREAD_LOCAL) != 0;
			if (!threadLocal)
    		{
				LockTable.checkLock( readFieldAccess.hashCode(), clock);
    		}
        	readFieldAccess.clear();
        }
    }
