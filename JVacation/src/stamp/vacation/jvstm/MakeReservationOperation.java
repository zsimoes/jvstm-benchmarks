...
public class MakeReservationOperation extends Operation
{
	...
	private class NestedWorker extends ParallelTask<Boolean>
	{
		...
		public NestedWorker(boolean readOnly, int min, int max)
		{
	...
	private void makeReservation(boolean readOnly)
	{
		...
		List<TransactionalTask<Boolean>> workers = new ArrayList<TransactionalTask<Boolean>>();
		//System.out.println("\t\tMakeReservation - running workers: " + (numQuery / queriesPerTx));
		for (n = 0; (n + queriesPerTx) < numQuery; n += queriesPerTx)
		{
			workers.add(new NestedWorker(readOnly, n, n + queriesPerTx));
		} /* for n */

		for (boolean bol : Transaction.current().manageNestedParallelTxs(workers))
		{
			if (bol)
			{
				isFound = true;
		...
		//System.out.println("\t    MakeReservation: finished nested (" + (numQuery / queriesPerTx) + ")");
