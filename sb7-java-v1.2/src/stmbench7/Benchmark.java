...
public class Benchmark
{
			...
			try
			{
				...
				} else
				{
					...
					if (currentArg.equals("-t"))
						Parameters.numTopLevel = Integer.parseInt(optionValue);
					else if (currentArg.equals("-n"))
					{
						int nest = Integer.parseInt(optionValue);
						if (nest > 0)
						{
							Parameters.numNested = nest;
							Parameters.nesting = true;
						} else {
							Parameters.numNested = 0;
							Parameters.nesting = false;
						}
	...
	private void printRunTimeParametersInformation()
	{
		...
		System.out.println("Number of threads: " + Parameters.numTopLevel + "\n" + "Length: " + Parameters.numSeconds
				+ " s\n" + "Workload: " + Parameters.workloadType + "\n" + "Synchronization method: "
				+ Parameters.synchronizationType + "\n" + "Long traversals "
				+ (Parameters.longTraversalsEnabled ? "enabled" : "disabled") + "\n"
				+ "Structural modification operations "
				+ (Parameters.structureModificationEnabled ? "enabled" : "disabled") + "\n" + "DesignObjFactory: "
				+ DesignObjFactory.instance.getClass().getName() + "\n" + "BackendFactory: "
				+ BackendFactory.instance.getClass().getName() + "\n" + "OperationExecutorFactory: "
				+ OperationExecutorFactory.instance.getClass().getName() + "\n" + "ThreadFactory: "
				+ ThreadFactory.instance.getClass().getName() + "\n" + "Sequential replay "
				+ (Parameters.sequentialReplayEnabled ? "enabled" : "disabled"));
	...
	{
		double shortTraversalsRatio = (double) Parameters.ShortTraversalsRatio / 100.0, operationsRatio = (double) Parameters.OperationsRatio / 100.0;

		double traversalsRatio, structuralModificationsRatio;
		if (Parameters.longTraversalsEnabled)
			traversalsRatio = (double) Parameters.TraversalsRatio / 100.0;
		else
			traversalsRatio = 0;
		...
		double readOnlyOperationsRatio = (double) Parameters.workloadType.readOnlyOperationsRatio / 100.0, updateOperationsRatio = 1.0 - readOnlyOperationsRatio;

		double sumRatios = traversalsRatio + shortTraversalsRatio + operationsRatio + structuralModificationsRatio
				* updateOperationsRatio;
		...
		OperationType.TRAVERSAL.probability = traversalsRatio * updateOperationsRatio / OperationType.TRAVERSAL.count;
		OperationType.TRAVERSAL_RO.probability = traversalsRatio * readOnlyOperationsRatio
				/ OperationType.TRAVERSAL_RO.count;
		...
		OperationType.OPERATION_RO.probability = operationsRatio * readOnlyOperationsRatio
				/ OperationType.OPERATION_RO.count;
	...
	private void setupStructures() throws InterruptedException
	{
		...
		benchThreads = new BenchThread[Parameters.numTopLevel];
		threads = new Thread[Parameters.numTopLevel];
		for (short threadNum = 0; threadNum < Parameters.numTopLevel; threadNum++)
		{
	...
	private void showStats()
	{
		...
		for (OperationType type : OperationType.values())
		{
			...
			System.out.println(alignText(type.toString(), 23) + ":  " + "expected = " + formatDouble(expectedRatio)
					+ "\tsuccessful = " + formatDouble(realRatio) + "\terror = " + formatDouble(error) + "\t(total = "
					+ formatDouble(tRealRatio) + "\terror = " + formatDouble(tError) + ")");
			totalError += error;
			totalTError += tError;
		}
		System.out.println();
		...
		for (OperationType type : OperationType.values())
		{
			...
			System.out.println(alignText(type.toString(), 23) + ":  " + "successful = " + type.successfulOperations
					+ "\tmaxttc = " + type.maxttc + "\tfailed = " + type.failedOperations + "\ttotal = "
					+ totalTypeOperations);
		}
		System.out.println();

		System.out.println("Total sample error: " + formatDouble(totalError) + "%" + " (" + formatDouble(totalTError)
				+ "% including failed)");
	...
	private void printSyntax()
	{
		String syntax = "Syntax:\n" + "java stmbench7.Benchmark [options] [-- stm-specific options]\n\n" + "Options:\n"
				+ "\t-t numTopLevelThreads -- set the number of  top-level threads (default: 1)\n"
				+ "\t-n numNestedThreads -- set the number of  nested threads (default: 0)\n"
