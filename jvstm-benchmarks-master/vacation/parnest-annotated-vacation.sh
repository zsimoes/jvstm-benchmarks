...
if [ $# -ne 2 ]; then
	echo "Pass a parameter with the path to the jvstm jar, and to the asm jar"
	exit
fi
...
java -cp $jvstmlib:$asmlib jvstm.atomic.ProcessAtomicAnnotations $vacation
java -cp $jvstmlib:$asmlib jvstm.atomic.ProcessParNestAnnotations $vacation
