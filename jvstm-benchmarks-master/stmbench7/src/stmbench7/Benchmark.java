...
public class Benchmark {
	...
	throws BenchmarkParametersException {
		int argNumber = 0;
		...
		while (argNumber < args.length) {
			...
			try {
				...
				else if (currentArg.equals("--no-rw-traversals")) Parameters.longReadWriteTraversalsEnabled = false;
		...
		if (workload != null) {
			if(workload.equals("ro")) Parameters.workloadType = Parameters.WorkloadType.READ_ONLY;
			else if (workload.equals("r")) Parameters.workloadType = Parameters.WorkloadType.READ_DOMINATED;
			else if (workload.equals("rw")) Parameters.workloadType = Parameters.WorkloadType.READ_WRITE;
			else if (workload.equals("w")) Parameters.workloadType = Parameters.WorkloadType.WRITE_DOMINATED;
			else throw new BenchmarkParametersException("Invalid workload type: " + workload);
		}

		if (synchType != null) {
			if (synchType.equals("coarse")) Parameters.synchronizationType = Parameters.SynchronizationType.LOCK_COARSE;
			else if (synchType.equals("medium")) Parameters.synchronizationType = Parameters.SynchronizationType.LOCK_MEDIUM;
			else if (synchType.equals("fine")) Parameters.synchronizationType = Parameters.SynchronizationType.LOCK_FINE;
			else if (synchType.equals("none")) Parameters.synchronizationType = Parameters.SynchronizationType.NONE;
			else if (synchType.equals("stm")) Parameters.synchronizationType = Parameters.SynchronizationType.STM;
		...
		switch (Parameters.synchronizationType) {
			...
			case STM:
				if (stmInitializerClassName != null) {
					try {
						synchMethodInitializerClass = (Class<? extends SynchMethodInitializer>) Class.forName(stmInitializerClassName);
					...
					throw new BenchmarkParametersException(
							"STM initializer class name not given" +
	...
	private void generateOperationCDF() {
		...
		if (Parameters.structureModificationEnabled) structuralModificationsRatio = (double) Parameters.StructuralModificationsRatio / 100.0;
		...
		OperationType.TRAVERSAL.probability =
			rwTraversalsRatio / OperationType.TRAVERSAL.count;
		OperationType.TRAVERSAL_RO.probability =
			roTraversalsRatio / OperationType.TRAVERSAL_RO.count;
		...
		if (!reexecution) {
			...
			for (OperationType type : OperationType.values()) {
				System.out.println(alignText(type.toString(), 23) + ": " + formatDouble(type.probability * type.count * 100));
			}
			System.out.println();
		}
	...
	private void checkOpacity() throws InterruptedException {
		...
		for (BenchThread thread : benchThreads) replayLog.addAll(thread.replayLog);
