...
public class Benchmark implements org.deuce.benchmark.Benchmark {
	...
	public void init(String[] args) {
		...
		if (args.length > 0) {
			...
			else if (args[0].equals("LinkedListJvstm"))
				m_set = new IntSetLinkedListJvstm();
			else if (args[0].equals("SkipListJvstm"))
				m_set = new IntSetSkipListJvstm();
			else if (args[0].equals("RBTreeJvstm"))
				m_set = new RBTreeJvstm();
			else if (args[0].equals("TreeMapJvstm"))
				m_set = new IntSetTreeMapJvstm();
			else
				error = true;
		} else
			error = true;
	...
	public String getStats(org.deuce.benchmark.BenchmarkThread[] threads) {
	    // try {
	    // System.out.println("max=" + jvstm.gc.GCTask.max);
	    // System.out.println("total=" + jvstm.gc.GCTask.total);
	    // System.out.println("count=" + jvstm.gc.GCTask.count);
	    // System.out.println(((float)jvstm.gc.GCTask.total)/jvstm.gc.GCTask.count);
	    // } catch (Throwable t) {
