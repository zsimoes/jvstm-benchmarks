...
/**
 * Creates an OperationExecutor object, which is used to execute the benchmark
 * operations. For the default implementation, see
 * stmbench7.impl.DefaultOperationExecutorFactory.
 */
@NonAtomic
public abstract class OperationExecutorFactory {
	...
	public static void executeSequentialOperation(final Operation op) throws InterruptedException {
		Thread opThread = ThreadFactory.instance.createThread(new Runnable() {
			public void run() {
					OperationExecutor operationExecutor = instance.createOperationExecutor(op);
					try {
