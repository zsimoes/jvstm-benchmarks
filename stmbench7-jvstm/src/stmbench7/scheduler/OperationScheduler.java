...
public class OperationScheduler implements Scheduler {
    ...
    // Singleton Pattern
    private static final OperationScheduler _scheduler = new OperationScheduler();
	...
	// while( next == null ) {
	// replenishOperations(2000);
	// next = _policy.decideNext(_queuedOperations, _computingOperations);
	// }
	...
	OperationId conflicter = OperationScheduler.getProfiler().getRegisteredOperation(ce.getTxNumber());
	if (conflicter != null)
